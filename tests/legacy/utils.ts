...
import { PerpetualsAccount, PerpetualsMarket } from "@sdk/packages";
import type { ObjectId, SuiAddress } from "@sdk/types";
...
export const LOT_SIZE = BigInt(1_000_000);
...
export async function getPerpetualsAccount(
	...
	const accountObj = await aftermathApi
		.Perpetuals()
		.fetchAccount({ accountId });
	const accCap = await aftermathApi.Perpetuals().fetchOwnedAccountCapsOfType({
		walletAddress,
...
export async function getPerpetualsMarket(
	...
	const marketParams = await aftermathApi
		.Perpetuals()
		.fetchMarketParams({ collateralCoinType: coinType, marketId });
	const marketState = await aftermathApi
		.Perpetuals()
		.fetchMarketState({ collateralCoinType: coinType, marketId });
	const orderbook = await aftermathApi
		.Perpetuals()
...
export async function createAndFetchAccountCap(
	...
	const tx = await aftermathApi.Perpetuals().buildCreateAccountTx({
	...
	const change = response.objectChanges?.find(
		isAccountCapCreated
