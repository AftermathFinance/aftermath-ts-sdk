...
describe("Perpetuals Tests", () => {
	test("Account Margin", async () => {
		...
		const initialOraclePrice0 = BigInt(10_000) * ONE_F18;
		...
		const onchainTime = Helpers.getObjectFields(
			await aftermathApi
				.Objects()
		...
		const market0 = await getPerpetualsMarket(
			aftermathApi,
			MARKET_ID0,
			usdcType
		);
		...
		const market1 = await getPerpetualsMarket(
			aftermathApi,
			MARKET_ID1,
			usdcType
		);
		...
		let accountUser2 = await getPerpetualsAccount(
			aftermathApi,
			await user2.toSuiAddress(),
			BigInt(1),
			usdcType
		);

		console.log("Place limit order market0");
		const orderSize = BigInt(5000);
