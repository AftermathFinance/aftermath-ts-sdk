...
import { initLoaderIfNeeded } from '../init'
...
export class Field<Name, Value> {
  ...
  static async fetch<Name, Value>(
    ...
    if (res.error) {
      throw new Error(`error fetching Field object at id "${id}": ${(res as any).error.tag}`)
    }
    if (
      res.data!.bcs!.dataType !== 'moveObject' ||
      !isField(res.data!.bcs && 'type' in res.data!.bcs ? res.data!.bcs!.type : '')
    ) {
    ...
    return Field.fromBcs(typeArgs as [Type, Type], (res as any).data!.bcs!.bcsBytes, 'base64')
  }
}
