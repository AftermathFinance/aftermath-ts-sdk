...
export class TreasuryCap {
  ...
  static async fetch(provider: JsonRpcProvider, id: ObjectId): Promise<TreasuryCap> {
    ...
    return TreasuryCap.fromBcs(typeArgs[0], (res as any).data!.bcs!.bcsBytes, 'base64')
  }
}
...
export class Coin {
  ...
  static async fetch(provider: JsonRpcProvider, id: ObjectId): Promise<Coin> {
    ...
    if (res.error) {
      throw new Error(`error fetching Coin object at id "${id}": ${(res as any).error.tag}`)
    }
    if (
      res.data!.bcs!.dataType !== 'moveObject' ||
      !isCoin(res.data!.bcs && 'type' in res.data!.bcs ? res.data!.bcs!.type : '')
    ) {
    ...
    return Coin.fromBcs(typeArgs[0], (res as any).data!.bcs!.bcsBytes, 'base64')
  }
}
