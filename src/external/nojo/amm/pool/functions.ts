...
export function deposit(
	...
	return tx.moveCall({
		...
		typeArguments: [typeArgs[0], typeArgs[1]],
		arguments: [
			is(args.pool, TransactionArgument)
				? args.pool
				: tx.object(args.pool),
			is(args.inputA, TransactionArgument)
				? args.inputA
				: tx.object(args.inputA),
			is(args.inputB, TransactionArgument)
				? args.inputB
				: tx.object(args.inputB),
	...
	return tx.moveCall({
		...
		arguments: [
			is(args.pool, TransactionArgument)
				? args.pool
				: tx.object(args.pool),
			is(args.lpIn, TransactionArgument)
				? args.lpIn
				: tx.object(args.lpIn),
