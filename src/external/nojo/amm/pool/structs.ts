...
bcs.registerStructType(`${PACKAGE_ID}::pool::PoolCreationEvent`, {
...
export class Pool {
	...
	static fromFieldsWithTypes(item: FieldsWithTypes): Pool {
		...
		return new Pool([typeA, typeB], {
			...
			adminFeeBalance: Balance.fromFieldsWithTypes(
				typeB,
	...
	static async fetch(provider: JsonRpcProvider, id: ObjectId): Promise<Pool> {
		...
		if (res.error) {
			throw new Error(
				`Error fetching Pool object at ${id}: ${res.error.code}`
			);
		}
		if (
			res.data!.bcs!.dataType !== "moveObject" ||
			!isPool(
				res.data!.bcs && "type" in res.data!.bcs
					? res.data!.bcs!.type
					: ""
			)
		) {
		...
		const { typeArgs } = parseTypeName(
			res.data!.bcs && "type" in res.data!.bcs ? res.data!.bcs!.type : ""
		);

		return this.fromBcs(
			typeArgs as [Type, Type],
			(res as any).data!.bcs!.bcsBytes,
			"base64"
		);
	}
}
...
bcs.registerStructType(`${PACKAGE_ID}::pool::PoolRegistry`, {
	...
	table: [
		"0x2::table::Table",
		`${PACKAGE_ID}::pool::PoolRegistryItem`,
		"bool",
	],
});
...
export class PoolRegistry {
	...
	static fromFields(fields: Record<string, any>): PoolRegistry {
		return new PoolRegistry({
			...
			table: Table.fromFields(
				[`${PACKAGE_ID}::pool::PoolRegistryItem`, "bool"],
				fields.table
			),
		});
	}
	...
	static fromBcs(
		...
		return PoolRegistry.fromFields(
			bcs.de(`${PACKAGE_ID}::pool::PoolRegistry`, data, encoding)
		);
	}
	...
	static fromSuiParsedData(content: SuiParsedData): PoolRegistry {
		...
		if (!isPoolRegistry(content.type)) {
			throw new Error(
				`object at ${content.fields.id.id} is not a PoolRegistry`
			);
		}
	...
	static async fetch(
		...
		if (res.error) {
			throw new Error(
				`Error fetching PoolRegistry object at ${id}: ${
					(res as any).error.tag
				}`
			);
		}
		if (
		...
		return this.fromBcs((res as any).data.bcs.bcsBytes, "base64");
	}
}
...
export class PoolRegistryItem {
	...
	static fromBcs(
		...
		return PoolRegistryItem.fromFields(
			bcs.de(poolRegistryItemType, data, encoding)
		);
	}
}
