...
export class NftAmmApi {
	...
	public buyTx = (inputs: {
		...
		const { tx, assetCoin, genericTypes, nftIds } = inputs;
		return tx.moveCall({
			...
			arguments: [
				...
				typeof assetCoin === "string"
					...
					: assetCoin,
				tx.makeMoveVec({
					objects: nftIds.map((id) => tx.object(id)),
					type: "ID",
				}),
	...
	public addWithdrawCommandToTransaction = (inputs: {
		...
		const { tx, lpCoin, genericTypes, nftIds } = inputs;
		return tx.moveCall({
			target: Helpers.transactions.createTxTarget(
				this.addresses.packages.nftAmm,
				inputs.withTransfer
