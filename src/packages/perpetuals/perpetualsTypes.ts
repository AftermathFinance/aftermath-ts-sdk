 ...
 *
 * Integrators register once globally and are assigned a numeric `integratorId`.
 * A user then approves an integrator by creating an integrator config with a
 * `maxIntegratorFee` (see
 * {@link ApiPerpetualsBuilderCodesCreateIntegratorConfigTxBody}). Orders placed
 * on that user's behalf attach the integrator id and the fee to charge here.
 *
 * The fee is charged on the notional filled by the order and follows the order,
 * so it applies to both taker and maker fills.
 */
export interface PerpetualsBuilderCodeParamaters {
	...
	/**
	 * Integrator fee (as a decimal) charged on the notional filled by this order.
	 *
	 * For example, 0.0005 represents a 0.05% fee. This value must not exceed
	 * the `maxIntegratorFee` that the user approved for this integrator. The fee
	 * follows the order, so it is collected on taker and maker fills alike.
	 */
...
export interface ApiPerpetualsMaxOrderSizeBody {
	 ...
	 * on the notional filled by this order. The fee follows the order, so it is
	 * charged on both taker and maker fills. The integrator must have been
	 * previously approved by the account owner, and the fee must not exceed the
	 * `maxIntegratorFee` the user approved for that integrator.
	 */
	builderCode?: PerpetualsBuilderCodeParamaters;
}
 ...
 * placed on their behalf. The user sets the `maxIntegratorFee` that the integrator
 * can charge per order; orders then attach a `builderCode` with the integrator id
 * and the fee to charge (see {@link PerpetualsBuilderCodeParamaters}).
 */
 ...
 * and if so, returns the `maxIntegratorFee` the integrator is authorized to charge.
 */
